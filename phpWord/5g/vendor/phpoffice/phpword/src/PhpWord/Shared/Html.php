    ...
    protected static function parseInlineStyle($node, $styles = array())
    {
        if (XML_ELEMENT_NODE == $node->nodeType) {
            ...
            foreach ($attributes as $attribute) {
               ...
               /* if(substr($val, 'width') !==false && $node->tagName == 'td')
                {
                    $val = 'width: '.intval(str_replace(array('width: ', '%'),array('',''), $val)).'%;';
                    $attribute->value = $val;
                }*/
                ...
                if($node->tagName == 'td')
                {
                    ...
                    if(count($temp) !== 1)
                    {
                        ...
                        {
                            if($arItem == '') continue;
                            if(strpos($arItem, 'width: ') !== false)
                            {
                                $newVal .= 'width: '.intval(str_replace(array('width: ', '%'),array('',''), $arItem)).'%;';
                            }
                            else
                            {
                                $newVal .= $arItem.';';
